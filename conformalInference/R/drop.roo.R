#' Prediction intervals for the excess error due to variable dropping
#'
#' Compute prediction intervals for the excess absolute error in dropping a
...
#'
#' @return A list with the following components: lo, up, vars, split, out.roo.
#'   The third component is a list of variables that were tested (i.e., dropped
#'   one at a time, to compute excess error). The first two are arrays of
#'   dimension n x length(vars) x m.
...
conformal.drop.roo = function(x, y, train.fun, predict.fun, varlist=0,
  ...
  if (length(varlist) == 1 && varlist == 0) vars = 1:p
  else {
    if (is.null(varlist) || !is.numeric(varlist) || min(varlist) < 1 ||
        max(varlist) > p || varlist != round(varlist)) {
      stop("varlist must be a list of integers between 1 and p")
    }
    vars = unique(varlist)
  }
  nvars = length(vars)
  lo = up = array(0,dim=c(n,nvars,m))
