...
# Regressogram
nbins = 80
s.split.cov = lm(as.numeric(out.split.cov) ~ cut(x0,nbins))$fitted
s.split.local.cov = lm(as.numeric(out.split.local.cov) ~ cut(x0,nbins))$fitted
s.split.len = lm(out.split.len ~ cut(x0,nbins))$fitted
s.split.local.len = lm(out.split.local.len ~ cut(x0,nbins))$fitted
