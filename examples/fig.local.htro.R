...
# heteroskedastic data (where it should help)
...
pdf(file="fig/sin.htro.split.pdf",w=w,h=h)
par(mar=mar)
...
pdf(file="fig/sin.htro.split.local.pdf",w=w,h=h)
par(mar=mar)
plot(c(), c(), xlab="X", ylab="Y", xlim=range(x0),
...
points(x, y, col=pcol)
graphics.off()
cat("fig/sin.htro.split.local.pdf\n")
...
pdf(file="fig/sin.htro.coverages.pdf",w=w,h=h)
par(mar=mar)
...
graphics.off()
cat("fig/sin.htro.coverages.pdf\n")

pdf(file="fig/sin.htro.lengths.pdf",w=w,h=h)
par(mar=mar)
